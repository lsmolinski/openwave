...
**Our current state**: the Combined Wolff-LaFreniere equation `ψ = 2A·sin(kr/2)·cos(kr/2-(ωt+φ))/r` was selected empirically for K=10 tetrahedron stability. It IS an exact solution to the 3D wave equation `∂²ψ/∂t² = c²∇²ψ`, which comes from the standard wave Lagrangian `L = ½(∂ψ/∂t)² - ½c²(∇ψ)²`. But the specific boundary conditions, source terms, and nonlinear extensions we use are NOT derived from a Lagrangian — they're modeling choices.
...
**Infrastructure**: standalone numpy script in `scripts_lagrangian/`, ~200 lines. 1D grid, simple PDE evolution. No M3/M4 refactor.
...
So the Lagrangian tests in `scripts_lagrangian/` would first validate *which* Lagrangian is right, and only then would we port the winning equation into wave_engine.py — potentially as a new method (M5) rather than modifying M3.
...
All 5 Lagrangian tests (scripts_lagrangian/) require a background vacuum field. They're inherently M2-like (field exists, defects are disturbances), not M3-like (no background). The scripts will initialize a uniform director field `n(x) = ẑ` as the vacuum, then create hedgehog defects as deformations.
...
**For numpy research scripts** (scripts_lagrangian/): infrastructure doesn't matter — we allocate whatever arrays we need. But for eventual GPU implementation (M5 on Taichi), it would extend M4's vector grid infrastructure.
...
**Infrastructure**: requires vector field (M4-like), time-domain PDE evolution (M2-like), nonlinear terms. Standalone numpy script in `scripts_lagrangian/`, ~300-400 lines. No M4 refactor needed.
...
**Infrastructure**: standalone numpy script in `scripts_lagrangian/`, ~300 lines. Time-domain PDE evolution (like M2 Laplacian mode) on a 3D scalar grid. No M3/M4 refactor.

...
- 🔶 M5 / LAGRANGIAN-FIELD METHOD implementation (dir `openwave/xperiments/m5_lagrangian_field/`) — see [3d_path_to_m5.md](3d_path_to_m5.md) for full sub-phase plan
  ...
  - 🔶 M5.2 Wave dynamics — **Close's Eq. 23** as the particle equation (preserves `∇·s = 0`, per Close's 2026-04-18 guidance) + Eq. 19 as V=0 linear limit + Klein-Gordon mass term; validate against Exp 4 dispersion; resonance-hunt amplitude sweep. Kernel-internal natural-unit scaling (`c=1, λ_C=1, ℏ=1`) lands here for the nonlinear couplings (deferred from M5.0f — linear kernels are dimensionally self-balancing and don't need it). **Progress (2026-05-12)**: ✅ Step 1 natural-unit scaffold (`constants.COMPTON_WAVELENGTH_REDUCED_ELECTRON_AM`; `m_freq_kg_rs` threaded through `V_psi`+`evolve_psi`), ✅ Step 2 KG mass term active (`V = ½m²|ψ|²`, `−(m·dt)²·ψ` in leapfrog), ⚠️ Step 3 defect-survival NEGATIVE (`research/m5_2_kg_defect_survival.py`: hedgehog collapses identically under V=0 and KG-electron — plain KG too weak at electron scale AND wrong potential shape; minimum at ψ=0 destabilizes |ψ|=1 hedgehog). External-comms trigger NOT met; Step 4 (Close Eq. 23 / LdG Mexican-hat) is the structural fix — proceeding with default choices since Close has not responded

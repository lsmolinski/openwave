...
- 🔶 M5 / LAGRANGIAN-FIELD METHOD implementation (dir `openwave/xperiments/m5_lagrangian_field/`) — see [3d_path_to_m5.md](3d_path_to_m5.md) for full sub-phase plan
  ...
  - 🔶 M5.1 Port topology from Exps 2, 3 — **5 of 7 tasks complete** as of 2026-05-11: ✅ `seed_vacuum`, ✅ `seed_hedgehog` (N-defect, port of Exp 2 weighted superposition), ✅ director-glyph visualizer (3-plane line glyphs, palette-encoded `1−n_z`, blueprint + ironbow options, half-arrowhead barbs at 36° sweep, VIZ_STRIDE consolidated with granules; design in `research_hub/3e_director_glyph_rendering.md`), ✅ Frank elastic energy density `compute_energyF_density` (M5.1 task 5; new `FieldObservables` class with `energyF_density_aJ` per 2026-05-11 SoC refactor; WAVE_MENU=5 "FRANK (Elastic)" flux-mesh coloring shipped alongside per `feedback_visual_rendering_priority.md`; all 5 regression tests pass in `research/m5_1_frank_energy.py` — vacuum baseline `\|F\|=0` exact, CV(F·r²)=0.0019 vs analytical `K/r²`, numpy parity at f32 round-off limit). Bonus fix: BC-bleed bug — every seeder now writes all 3 leapfrog buffers per `feedback_triple_buffer_bc.md` memory. 🚧 next: gradient-descent relaxation (task 6, port of Exp 2), then 1/d Coulomb gating test (task 7), winding-number tracker (task 8)

...
## 🔶 PHASE 3: LAGRANGIAN FRAMEWORK — see [3_LAGRANGIAN_FRAMEWORK.md](../openwave/xperiments/m5_lagrangian_field/research/3_LAGRANGIAN_FRAMEWORK.md) · [3a_lagrangian_experiments.md](../openwave/xperiments/m5_lagrangian_field/research/3a_lagrangian_experiments.md) · [3b_concept_review.md](../openwave/xperiments/m5_lagrangian_field/research/3b_concept_review.md) · [3c_topological_defect.md](../openwave/xperiments/m5_lagrangian_field/research/3c_topological_defect.md) · [3d_path_to_m5.md](../openwave/xperiments/m5_lagrangian_field/research/3d_path_to_m5.md)
...
- ✅ Winning combination selected (2026-04-17): topology (hedgehog + winding) + Klein-Gordon wave dynamics + Close's Eq. 19 as base vector wave + M3 near-field standing waves + Skyrme stabilizer. Full recipe in [3a § Winning Approach](../openwave/xperiments/m5_lagrangian_field/research/3a_lagrangian_experiments.md#winning-approach-for-m5)
- 🔶 M5 / LAGRANGIAN-FIELD METHOD implementation (dir `openwave/xperiments/m5_lagrangian_field/`) — see [3d_path_to_m5.md](../openwave/xperiments/m5_lagrangian_field/research/3d_path_to_m5.md) for full sub-phase plan
  ...
  - ✅ M5.1 Port topology from Exps 2, 3 — **8 of 8 tasks complete** as of 2026-05-11: ✅ `seed_vacuum`, ✅ `seed_hedgehog` (N-defect), ✅ director-glyph visualizer (3-plane line glyphs, palette `1−n_z`, blueprint+ironbow, half-arrowhead barbs, VIZ_STRIDE consolidated), ✅ Frank elastic `compute_energyF_density` + WAVE_MENU=5 (new `FieldObservables` class), ✅ gradient-descent `relax_director_step` (auto-relax on seed), ✅ **1/d Coulomb gating test (M5.2 gate)** — R² = 0.978 attractive + monotone (threshold 0.95; Dirichlet BC limits vs periodic) + **visual confirmation** in [`../openwave/xperiments/m5_lagrangian_field/research/3f_coulomb_visual_geometry.md`](../openwave/xperiments/m5_lagrangian_field/research/3f_coulomb_visual_geometry.md) (dumbbell F-density bridge for opposite charges; pinched perpendicular splay for same charges — matches classical EM field-line geometry, derived from pure topology), ✅ winding-number tracker `compute_winding_number` (Q=±0.996 on seeded ±1 hedgehogs). **Bonus discovery during task 8**: same-direction-pin + Dirichlet-BC setup loses topology after ~100 relax steps — Coulomb test still works (blend-zone elastic dominates F(d)), but defect stability needs M5.5 Skyrme to fully fix. Winding tracker now serves as the diagnostic gate for that issue during M5.2. **M5.1 effectively complete; M5.2 unblocked.**
  - ⚠️ M5.2 V(ψ) escalation on Vector(3) — **CLOSED as negative result (2026-05-12)**. Tested V = 0, KG mass `½m²|ψ|²`, φ⁴ Mexican-hat `¼λ(|ψ|²−1)²`, biharmonic `½κ|∇²ψ|²`. All four collapse Q identically at step 4-5 of `evolve_psi` regardless of pre-relax. Empirical diagnosis: seeded hedgehog is not a soliton of the dynamic equation. **Updated diagnosis after re-reading Duda paper (arxiv:2108.07896)**: static stable solitons don't exist in this framework — the "particle" is a time-periodic resonance (Zitterbewegung clock), and the field must be matrix `M = ODO^T` (biaxial nematic), not Vector(3) ψ. Triple-confirmed by Duda paper Fig. 10, Robert Close email, and Werbos chaoiton paper. Captured in [`../openwave/xperiments/m5_lagrangian_field/research/3g_lagrangian_roadblocks.md`](../openwave/xperiments/m5_lagrangian_field/research/3g_lagrangian_roadblocks.md) and memories `feedback_no_static_solitons` + `reference_duda_lcb_paper`. **Phase moves to "informative negative" — not blocking forward progress.**
  - 🔶 M5.3 **Direction review** (next active phase, started 2026-05-12). Study + decide before code commitment. Tasks: (a) re-read Duda paper §III-V (matrix field, V_LG, Eq. 42 4D Skyrme); (b) review Taichi storage/layout for matrix-field migration (`docs.taichi-lang.org/docs/field`, `/layout`, `/sparse`); (c) sandbox smoke test of Close's resonance protocol on existing Vector(3) substrate (informational, not gating); (d) feasibility study: matrix field `M = ODO^T` in Taichi (storage, kernel cost); (e) thermal prerequisites analysis — confirm M5.7 is sufficient foundation for M5.10 to start in parallel with M5.8/M5.9; (f) wait for Duda reply to 2026-05-12 outreach; (g) decision document locking M5.4+ plan. Exit when M5.4 implementation plan is concrete.
  - [ ] M5.4 **Matrix-field substrate migration**. Replace Vector(3) ψ with `M(x) = O(x) D O^T(x)` real symmetric 3×3 matrix field. Storage redesign in Taichi; operators (matrix commutator `[M_μ, M_ν]`, antisymmetric `A_μ = [M, ∂_μ M]`); reproduce M5.1 Coulomb on the new substrate. Lands deferred-from-M5.0g physical-energy scaling.
  - [ ] M5.5 **Paper Lagrangian + V(M)**. Implement Duda Eq. 18 action `L = Σ ‖F_μ0‖² − Σ ‖F_μν‖² − V(M)` with Eq. 13 Higgs-like `V_LG = a Tr(M²) − b Tr(M³) + c (Tr(M²))²` (or simpler eigenvalue-preference variant). Faber regularization integrated to activate V. Subsumes old "Skyrme stabilizer" phase since Eq. 42 4D Skyrme-like kinetic is the same family.
  - [ ] M5.6 **Biaxial twist + KG emergence**. With `Λ = (1, δ, 0)` and `δ ~ ℏ`, reproduce paper Fig. 9 — show Klein-Gordon-like equation emerges automatically from twist dynamics (NOT added as V_psi term). Maxwell-like equations emerge for tilt components.
  - [ ] M5.7 **Resonance hunt (Close protocol)**. `l = 1` harmonic seed on biaxial substrate, sweep `A/λ ∈ {0.5, 1, 2}`, measure energy-localization lifetime. First metastable resonance — "first long-lived particle in M5". **UNBLOCKS M5.10 thermal work** (drive infrastructure ready). Literature anchor: BEC vortex kinetics (Duda 2026-05-13 cite, PRA `10.1103/2msv-lk1m`) — compact-manifold geometry is a third Derrick escape alongside topology + time-periodicity.
  ...
  - [ ] M5.10 **Thermal energy — SABER tie-in (MAIN GOAL)**. Sub-phases:
    - [ ] M5.10a Drive infrastructure on top of M5.7 metastable resonance
    - [ ] M5.10b (A, ω) measurement under driven excitation; modulation hypothesis test
    - [ ] M5.10c+ TBD per SABER scope (see `project_thermal_amplitude_hypothesis` memory)

    Starts as soon as M5.4-M5.7 land; runs in parallel with M5.8/M5.9. Empirical falsification (or validation) of the SABER thermal-amplitude hypothesis.
...
- [ ] **Revisit Energy Layers hierarchy** with Lagrangian perspective — insert **Layer 0: Vacuum at rest** before Layer 1; rewrite Layer 1-3 semantics in [2_ENERGY_LAYERS.md](2_ENERGY_LAYERS.md), [0_OVERVIEW.md](0_OVERVIEW.md), and [3z_review_energy_layers.md](../openwave/xperiments/m5_lagrangian_field/research/3z_review_energy_layers.md) (the content was moved out of the README pending re-evaluation; once revised, decide whether to put it back) — rationale in [3b_concept_review.md](../openwave/xperiments/m5_lagrangian_field/research/3b_concept_review.md)

## [PHASE 4: TIME DYNAMICS](../openwave/xperiments/m5_lagrangian_field/research/5_TIME_DYNAMICS.md)

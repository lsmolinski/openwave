...
- 🔶 M5 / LAGRANGIAN-FIELD METHOD implementation (dir `openwave/xperiments/m5_lagrangian_field/`) — see [3d_path_to_m5.md](3d_path_to_m5.md) for full sub-phase plan
  ...
  - 🔶 M5.1 Port topology from Exps 2, 3 — **3 of 7 tasks complete** as of 2026-05-09: ✅ `seed_vacuum`, ✅ `seed_hedgehog` (N-defect, port of Exp 2 weighted superposition), ✅ director-glyph visualizer (3-plane line glyphs, palette-encoded `1−n_z`, blueprint + ironbow options, VIZ_STRIDE consolidated with granules; design in `research_hub/3e_director_glyph_rendering.md`). Bonus fix: BC-bleed bug — every seeder now writes all 3 leapfrog buffers per `feedback_triple_buffer_bc.md` memory. 🚧 next: Frank energy density (task 5), then gradient-descent relaxation, 1/d Coulomb gating test, winding-number tracker

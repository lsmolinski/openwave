...
| 383³ (56M voxels) | 19.4 | 51 | ✅ at edge but passes |
  ...
  Per-kernel breakdown is roughly stable across grid sizes: `swap_buffers` 34 %, `update_trackers_psi` 24 %, `propagate_psi` 23 %, `compute_energyH_density` 18 %. `sample_avg_trackers` (every-60-frames cadence) is negligible (~0.06 ms/step amortized).

...
- ✅ Frank Elastic Energy density `H_F = (K/2)·|∇n̂|²` — per-voxel kernel `compute_energyF_density` in `lagrangian_engine.py` mirroring `compute_energyH_density`'s gradient stencil (sans c² and sans kinetic/potential). Writes to `Trackers.energyF_density_aJ`. Global per-voxel mean populated by the existing 3-plane `sample_avg_trackers` pass (extended to 4 observables: amp / freq / energy_H / energy_F). `K_FRANK = 1.0` hardcoded as Exp 2 baseline; physical elastic constants land in M5.6. **Rendering integration**: `WAVE_MENU = 5` ("FRANK (Elastic)") added to the flux-mesh menu — ironbow palette, defect cores light up brightest, vacuum dark; dashboard shows global mean. Per `feedback_visual_rendering_priority.md` memory, visual integration shipped in the same PR as the kernel, not deferred. Volume integral `E(d) = ∫ H_F d³r` for the Coulomb fit (task 7) is the trivial `mean × voxel_count` derivation, same pattern as `energy_total_H`.

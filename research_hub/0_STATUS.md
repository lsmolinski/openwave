...
**Current status**: 3/8 passed ✅ + 1/8 mixed ⚠️ (Exp 5, see below); remaining 4 (Exps 4, 6, 7, 8) pending. Exp 5 revealed that the documented Combined W-L product form `2A·sin(kr/2)·cos(kr/2−(ωt+φ))/r` is NOT a free-wave solution — its quadrature term `(1−cos(kr))/r·sin(ωt+φ)` leaves a residual `−A·c²k²·sin(ωt+φ)/r ≠ 0` when substituted into the d'Alembertian. The M4 code's equivalent *sum form* `A·[sin(kr+ωt+φ)+sin(kr−ωt−φ)]/(kr)` IS a valid free-wave solution. Smolinski Ψ³ derivation from the quartic Lagrangian and Noether energy density both confirmed. Once complete, select winning combination → implement M5 / LAGRANGIAN-WAVE METHOD on the production Taichi engine. Conceptual framework review in [3a_concept_review.md](3a_concept_review.md) — notably includes the proposal to insert **Layer 0: Vacuum at rest** before Layer 1 in the Energy Layers hierarchy (see [2_ENERGY_LAYERS.md](2_ENERGY_LAYERS.md), [0_OVERVIEW.md](0_OVERVIEW.md), [README.md](../README.md)).

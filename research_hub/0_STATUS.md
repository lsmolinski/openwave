...
**Current status**: Phase 3 sandbox **complete** (all 8 experiments run, 2026-04-16 / 2026-04-17). 4 ✅ (Exps 1, 2, 3, 4) + 3 ⚠️ (Exps 5, 6, 7) + 1 ❌ (Exp 8). **Net verdict**: topology is the load-bearing ingredient (charge quantization, far-field Coulomb); pure nonlinearity fails for K-selectivity; Close's actual vector wave equation (Exp 7 v2 with the paper obtained) gives massless transverse waves consistent with the Dirac-equation factoring but does not produce static solitons from harmonic seeds (consistent with his framework — particles are plane-wave bispinors). The winning recipe for M5: **topology + Close's vector wave equation + Skyrme stabilizer + M3 near-field standing-wave lock-in**. Full Overall Conclusions in [3b_lagrangian_experiments.md § OVERALL CONCLUSIONS](3b_lagrangian_experiments.md#overall-conclusions). Next phase: begin **M5.0 scaffold** per [3c_path_to_m5.md](3c_path_to_m5.md). Exp 5 revealed that the documented Combined W-L product form `2A·sin(kr/2)·cos(kr/2−(ωt+φ))/r` is NOT a free-wave solution — its quadrature term `(1−cos(kr))/r·sin(ωt+φ)` leaves a residual `−A·c²k²·sin(ωt+φ)/r ≠ 0` when substituted into the d'Alembertian. The M4 code's equivalent *sum form* `A·[sin(kr+ωt+φ)+sin(kr−ωt−φ)]/(kr)` IS a valid free-wave solution. Smolinski Ψ³ derivation from the quartic Lagrangian and Noether energy density both confirmed. Exp 8 (2026-04-17) falsified the claim that Smolinski's Ψ³ *by itself* produces K-selectivity — bumps at K=1, 2, 4, 6, 8 show nearly identical peak-amplitude dynamics at every κ tested. Confirms Duda's position: topology + nonlinearity are BOTH needed; neither alone suffices. Exp 4 (2026-04-17) confirmed massive Klein-Gordon dispersion `ω² = c²k² + m²` to R² = 0.999982 across 9 wavenumber modes, with slope c² within 0.05% and mass gap within 1.3% — validates that quadratic potentials in the Lagrangian automatically give waves a rest mass (Duda's Zitterbewegung / time-crystal mechanism at linear order). Once complete, select winning combination → implement M5 / LAGRANGIAN-WAVE METHOD on the production Taichi engine. Conceptual framework review in [3a_concept_review.md](3a_concept_review.md) — notably includes the proposal to insert **Layer 0: Vacuum at rest** before Layer 1 in the Energy Layers hierarchy (see [2_ENERGY_LAYERS.md](2_ENERGY_LAYERS.md), [0_OVERVIEW.md](0_OVERVIEW.md), [README.md](../README.md)).
...
### Quick Tests Planned (sandbox_phase2_lagrangian/)

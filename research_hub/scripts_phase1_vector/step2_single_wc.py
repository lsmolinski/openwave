...
def main():
    ...
    for eta in ETA_VALUES:
        ...
        print(f"\n{'─' * 65}")
        print(f"  eta = {eta_label}  (L out = {np.sqrt(1-eta)*100:.1f}%, T out = {np.sqrt(eta)*100:.1f}%)")
        print(f"{'─' * 65}")

        # Compute WC out-wave
        P_wc = compute_wc_outwave(coords, WC_POS, WC_PHASE, WC_AMPLITUDE, K0,
                                  eta, SPIN_SIGN, SPIN_AXIS)
        ...
        print(f"  {'r/lam':>6s}  {'E_total':>10s}  {'E_base':>10s}  {'E_L':>10s}  {'E_T':>10s}  {'E_L/E':>6s}  {'concen':>8s}")
        ...
        for i in range(0, len(r_lam), 3):  # Print every 3rd bin
            if E_tot_r[i] > 0 and E_base_r[i] > 0:
                ...
                print(f"  {r_lam[i]:6.2f}  {E_tot_r[i]:10.4e}  {E_base_r[i]:10.4e}"
    ...
    P_wc_alpha = compute_wc_outwave(coords, WC_POS, WC_PHASE, WC_AMPLITUDE, K0,
                                    ALPHA, SPIN_SIGN, SPIN_AXIS)
    ...
    checks = [
        ...
        ("E_L + E_T = E_total (decomposition exact)",
         True,
         "by construction"),
    ]

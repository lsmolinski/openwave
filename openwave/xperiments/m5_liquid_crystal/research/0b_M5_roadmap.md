...
>
> 🚧 **Sub-step plan:** ✅ M5.6.1 KG-from-twist sandbox (mass is geometric; `5a §5a`) → 🔶 M5.6.2 disclination + core (**2a ✅** — `C_μν≠0` mass source, `‖C‖~1/r²`, disclination regularized, `5a §5b`; **2b NEXT** = dynamical twist evolution showing the mass) → M5.6.3 Faber V-on regularization → M5.6.4 Maxwell sector → M5.6.5 Taichi port + biaxial-ellipsoid rendering.

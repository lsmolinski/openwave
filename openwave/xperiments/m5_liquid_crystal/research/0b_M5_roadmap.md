...
>
> 🚧 **Sub-step plan:** ✅ M5.6.1 KG-from-twist sandbox (mass is geometric; `5a §5a`) → ✅ M5.6.2 disclination + core (`C_μν≠0` mass source `~1/r²`, disclination regularized [2a]; biaxial hedgehog dynamically SOURCES its own twist + restoring mass, full-H conserved [2b]; `5a §5b`) → ✅ M5.6.3 Faber regularization (Faber MTF ported, `E₀=α_f ℏc·π/(4r₀)` verified [3a]; `Λ=q₀⁶/r₀⁴` mapped to `V(M)` via spatially-melting `D(r)` — both singularities melt to isotropic, mass pinned `E∝1/r₀` [3b]; `5a §5c`) → ✅ M5.6.4 Maxwell sector (EM-from-tilts both routes: hydro↔EM dictionary reproduces Maxwell to machine precision [4a]; Faber `R=Γ×Γ` is a closed field strength — Maurer–Cartan ⇒ homogeneous Maxwell — abelian Coulomb `~1/r²` far + running-coupling onset at `r₀` [4b]; `5a §5d`) → 🔶 **M5.6.5 Taichi port + biaxial-ellipsoid rendering (NEXT)**.

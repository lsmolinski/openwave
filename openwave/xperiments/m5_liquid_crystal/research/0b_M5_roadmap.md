...
> 🚧 **NEXT: M5.6** (biaxial twist + KG emergence) — the organizing phase. M5.5.5 (Faber EM Lagrangian + EM-from-tilts cross-check, `4a §11b`) **folds into M5.6**: the EM/Maxwell-from-tilts sector overlaps M5.6's "high-energy tilt modes obey Maxwell" verification (Eq.37–38), so it runs there rather than as a standalone sub-step.
...
- [ ] **Faber's explicit quantized-EM Lagrangian** (`4a §11b.2`): `Γ_i = (∂_i u)×u`, `R_μν = Γ_μ×Γ_ν`, `L_EM = −(αℏc/16π) R_μν R^μν` with `F_μν ~ R*_μν` (E↔B dual) — port alongside the LdG potential. **→ folded into M5.6** (runs with the Maxwell-sector verification, Eq.37–38).
- [ ] **EM-from-tilts cross-check** (`4a §11b.1`) — the superfluid hydrodynamics↔EM dictionary (vorticity `ω=∇×u ↔ B=∇×A`, Coriolis↔Lorentz, Lorenz-gauge parallel) is an independent route to Maxwell; sanity check on the matrix-curvature EM derivation. **→ folded into M5.6** (overlaps M5.6's "high-energy tilt modes obey Maxwell" task).
...
- 🔶 **Apply physical-energy-scaling factor** — `compute_energyH_density_M` (M5.5.4) carries an `e_scale` hook, currently `e_scale=1.0` (bare "(rel.)" units). The physical factor (`ρ_medium × voxel_volume_am³ × INTERNAL_ENERGY_TO_AJ`) is a CALIBRATION tied to a reference mass scale → **deferred to M5.9** (absolute energy set by matching lepton masses).
...
- [ ] Verify high-energy tilt modes obey Maxwell-like equations (paper Eq. 37 + 38) — **absorbs M5.5.5**: Faber's explicit quantized-EM Lagrangian (`L_EM = −(αℏc/16π) R_μν R^μν`, `4a §11b.2`) + the EM-from-tilts hydrodynamics↔Maxwell cross-check (vorticity↔B, Coriolis↔Lorentz, `4a §11b.1`)

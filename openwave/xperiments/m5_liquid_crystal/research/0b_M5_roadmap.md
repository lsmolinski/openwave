...
> 🚧 **NEXT: M5.5.3** — add `V(M)` (Eq.12 eigenvalue-preference → Eq.13 LdG) + Faber regularization to the evolution; then the **Taichi port** (wire the matrix leapfrog into the launcher `compute_propagation` no-op + matrix Hamiltonian — the M5.4 carry-overs). The biaxial-hedgehog KG dispersion + disclination handling is **M5.6**.

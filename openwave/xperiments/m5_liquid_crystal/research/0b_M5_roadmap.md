...
>
> 🚧 **Sub-step plan:** ✅ M5.6.1 KG-from-twist sandbox (mass is geometric; `5a §5a`) → ✅ M5.6.2 disclination + core (`C_μν≠0` mass source `~1/r²`, disclination regularized [2a]; biaxial hedgehog dynamically SOURCES its own twist + restoring mass, full-H conserved [2b]; `5a §5b`) → ✅ M5.6.3 Faber regularization (Faber MTF ported, `E₀=α_f ℏc·π/(4r₀)` verified [3a]; `Λ=q₀⁶/r₀⁴` mapped to `V(M)` via spatially-melting `D(r)` — both singularities melt to isotropic, mass pinned `E∝1/r₀` [3b]; `5a §5c`) → 🔶 M5.6.4 Maxwell sector (**4a ✅** — hydro↔EM dictionary reproduces Maxwell to machine precision: ∇·B=0, Gauss charge identity, Faraday, Lorentz force; `5a §5d`; **4b NEXT** = Faber matrix-curvature `R=Γ×Γ`, abelian-at-long-range + running-coupling onset) → M5.6.5 Taichi port + biaxial-ellipsoid rendering.
...
- 🔶 **M5.6.4 — Maxwell sector (folded M5.5.5).** Verify high-energy tilt modes obey Maxwell-like equations (paper Eq. 37 + 38). **Structural note:** Faber's `R_μν=Γ_μ×Γ_ν` is non-abelian (`*F_μν∝R_μν`, dual); ordinary Maxwell is the long-range abelian limit, short-range non-abelian = the running-coupling (`α_sol(d)`, §5c). **4a ✅** (`sandbox_v5/m5_6_4a_hydro_em.py`, `5a §5d`): the hydro↔EM dictionary (vorticity `ω=∇×u↔B`, Lamb `l=ω×u↔E`) reproduces Maxwell to machine precision — `∇·B=0`, the Gauss charge identity `∇·l=u·(∇×ω)−‖ω‖²`, Faraday `∂_tω=−∇×l`, Lorentz `F∝v×ω` (Coriolis). The clean abelian route. **4b NEXT** = Faber matrix-curvature route (`R=Γ×Γ` → abelian `F` at long range; non-abelian correction = running-coupling onset, ties to 3a's `α_sol(d)`); Faber's `L_EM=−(αℏc/16π)R·R` (`4a §11b.2`).

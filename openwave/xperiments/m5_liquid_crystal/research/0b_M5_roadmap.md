...
> 🚧 **NEXT: M5.5.4 Taichi port** — wire the validated matrix leapfrog into the launcher `compute_propagation` no-op (makes "Evolve PDE" live) + the matrix Hamiltonian `ℋ=‖Ṁ‖²+‖∇M‖²+V(M)` into `compute_energyH_density` + physical-energy scaling (the M5.4 carry-overs). Then M5.5.5 EM-from-tilts cross-check. The biaxial-hedgehog KG dispersion + disclination = **M5.6**.

- 🔶 Implement Eq. 18 action on the M field from M5.4 — **sandbox ✅** (`sandbox_v4`: operators `m5_5_1`, evolution + energy conservation `m5_5_2`, V + regularization `m5_5_3`). **Production Taichi port = M5.5.4** (pending).
...
- 🔶 **Faber regularization** — **mechanism validated** (`m5_5_3`): Eq.18 `F²` + V are Derrick-stable (`E(L)=E_F/L+E_V·L³` → finite core + mass; F² IS the Skyrme-family kinetic, no extra term). Faber's **exact** running-coupling scheme (`reference_faber_regularization.md`) = **Q8** (Duda open) — port + adapt in M5.5.5/M5.6.
- [ ] **Faber's explicit quantized-EM Lagrangian** (`4a §11b.2`): `Γ_i = (∂_i u)×u`, `R_μν = Γ_μ×Γ_ν`, `L_EM = −(αℏc/16π) R_μν R^μν` with `F_μν ~ R*_μν` (E↔B dual) — port alongside the LdG potential. **(M5.5.5, pending)**
- [ ] **EM-from-tilts cross-check** (`4a §11b.1`) — the superfluid hydrodynamics↔EM dictionary (vorticity `ω=∇×u ↔ B=∇×A`, Coriolis↔Lorentz, Lorenz-gauge parallel) is an independent route to Maxwell; sanity check on the matrix-curvature EM derivation. **(M5.5.5, pending)**
...
- [ ] **Apply physical-energy-scaling factor** — **M5.5.4 (Taichi port)**: the factor (`ρ_medium × voxel_volume_am³ × INTERNAL_ENERGY_TO_AJ`) goes on the *matrix* energyH (`‖Ṁ‖² + ‖∇M‖² + V(M)`) when it replaces the retiring ψ `compute_energyH_density`. (Sandbox uses bare units.)
- [ ] **⤺ M5.4 LIVE-PATH CARRY-OVER (revisit the cleanup no-ops)** — M5.4 left the matrix field STATIC, parking two live-path stubs for this phase: **(a)** the launcher's `compute_propagation` is a **no-op** awaiting the Eq.18 matrix leapfrog — wiring the matrix evolution in here is what makes the GUI "Evolve PDE" button actually evolve M; **(b)** `compute_energyH_density` + WAVE_MENU=4 ("ENERGY Hamiltonian") currently show a uniform `¼λ` placeholder (they still read the dormant ψ buffer, which is 0 for matrix xperiments) — replace with the matrix Hamiltonian `‖Ṁ‖²+‖∇M‖²+V(M)` and the physical-energy-scaling factor above. Both are deferred M5.4-cleanup items to close out here.
...
- [ ] Verify low-energy twist mode `ψ` obeys the Klein-Gordon-like equation with position-dependent (hedgehog) mass — confirmed closed form in [`5a_lagrangian_evolution.md` §5](5a_lagrangian_evolution.md): `2∂_tt ψ = [(∇−Â)² + (Â·∇/‖Â‖)²] ψ`, `Â^hedg = (x,y,z)/r²`. Port directly — no need to re-derive.
...
- [ ] **⤺ M5.5.2 CARRY-OVER (the KG mass gap + biaxial complications handed off here)** — M5.5.2 validated the action-derived twist-evolution MACHINERY + energy conservation (`sandbox_v4/m5_5_2_twist_evolution.py`, 0.21% drift) on a smooth single-generator background where `C_μν=0` (no mass gap). M5.6 inherits three things to make the KG mass gap appear: **(a)** use the **biaxial hedgehog** background (multiple generators → `C_μν = [M_μ^bg,M_ν^bg] ≠ 0`, the gauge source that gives the mass term); **(b)** handle the hedgehog's **z-axis DISCLINATION singularity** (`δ≠0` ⇒ `e_Φ~1/ρ`, hairy-ball — unavoidable for biaxial) + the core, e.g. mask/regularize (cf. M5.1's soft core); **(c)** damp the **amplitude growth at the `K→0` active-region edge** (kinetic-metric-degeneracy artifact seen in M5.5.2). Build on the validated `m5_5_2` leapfrog. See [`5a §9` + §8 row 5.5.2](5a_lagrangian_evolution.md).
- [ ] Review the "Rendering-stack repurposing" from M5.4 — build the biaxial-ellipsoid **surface** granule (deferred from M5.4 step 5, [`4b § Decisions taken`](4b_rendering_features.md)): now that `δ≠g` makes the order parameter genuinely biaxial, the prolate→biaxial ellipsoid surface mesh becomes meaningful (its minor axes are no longer degenerate)

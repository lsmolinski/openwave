...
| Integrator | conserved-`p_Θ` CC (sandbox ✅) → stable-region linearized field (sandbox ✅) → **the full nonlinear signed system: ONLY the spectral-projection constrained kernel is stable** (2c-1, f64: per-voxel positive-inertia eigenprojection of `A(Ṁ)=4Σ[η[Ṁ,M_i]η,M_i]` + `P`-projection every step + global-(α,3) clamp + bounded-energy guard). **VERDICT (2c-2): any cheap positive inertia (simple `½‖Ṁ‖²`, diagonal faithful-lite) under the signed flux has slow growing modes — heuristics (mask, smoothing, V-pinning) only stretch the blow-up timescale 120→250→700 steps** | ✅ 2c-1 (f64) · 🚧 Taichi port (Option B) |
...
| 🔶 M5.8.2c — production port (Steps 1–3 ✅: 2c-1 spike, Taichi port, headless 27/27 + GUI safe-v1 verified; the SIGNED dynamics OFF pending Option B) | the time axis is LIVE in production (safe v1, working recipe above); the Minkowski-signed flux requires **Option B — the constrained spectral-projection kernel in Taichi** (per-voxel 10×10 Jacobi, own implementation NOT `ti.sym_eig`; ~5–10× step cost). **Carries the three open physics gates** (roadmap 2c): G-2c-1 the defect holds ITSELF together (2c-1 first answer: held 900 steps, decisive M5.7-config re-run pending); G-2c-2 fuel-shell saturation (2c-1: H plateaued — first positive); G-2c-3 ω self-selection (2c-1: kicked clock holds, spontaneous start machine-zero — probe/symmetry diagnosis pending) | 🚧 Option B NEXT |

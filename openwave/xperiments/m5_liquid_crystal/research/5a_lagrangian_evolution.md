...
| Integrator | conserved-`p_Θ` CC on the ansatz manifold (sandbox ✅) → stable-region linearized field evolution (sandbox ✅, drift dt-flat = open-boundary exchange, cut-free twin 1.1%) → **the full nonlinear self-consistent constrained scheme = 2c** (the Q-PD mask as the projection mechanism + bounded-energy guard §10c; **never expose the global dressing amplitude as a free coherent DoF** — the 2b-1 runaway channel) | ✅ CC + stable-field · 🚧 2c full |
...
| 🚧 M5.8.2c — production port | the faithful 4D kinetic + constrained stepper into `engine2_pde.py`; deliberately cross the M5.8.1 time-freeze clamp + spatial-block-V boundary; do NOT give the global dressing amplitude a free coherent DoF (the 2b-1 ghost channel); the constraint mechanism is now concrete: the per-voxel `Q(∇ψ)`-PD stable mask + bounded-energy guard on the fuel shell. **Carries the three open physics gates** (roadmap 2c): G-2c-1 the defect holds ITSELF together (the money shot proper — first self-consistent evolution, resolution-confirmed); G-2c-2 fuel-shell saturation (the λ≈15.6/t runaway bounded by nonlinearity — currently a hypothesis); G-2c-3 ω self-selection (spontaneous spin-up, the time-crystal claim proper in 3+1D) | NEXT, Rodrigo-gated |

...
| 🚧 M5.8.2c — production port | the faithful 4D kinetic + constrained stepper into `engine2_pde.py`; deliberately cross the M5.8.1 time-freeze clamp + spatial-block-V boundary; do NOT give the global dressing amplitude a free coherent DoF (the 2b-1 ghost channel); the constraint mechanism is now concrete: the per-voxel `Q(∇ψ)`-PD stable mask + bounded-energy guard on the fuel shell | NEXT, Rodrigo-gated |

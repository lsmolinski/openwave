...
| The Minkowski signature SUSTAINS the clock at full nonlinearity (saturated regime) — the time-crystal ENGINE | ✅ kill-control twin (M5.8.2e, same seed+kick, 24k steps): Euclid clock DECAYS (1.4e-2 → 6.7e-3) while Minkowski+quartic clock GROWS (7.6e-3 → 2.4e-2); the saturated state is STRICTLY PERIODIC (ω₀=0.262 + exact 2-5× harmonic comb, breathing period τ=24) | `m5_8_2e_invariant_matrix.py` (euclid + omega modes) |
...
| --- | --- | --- |
| The defect holds ITSELF together (the M5.7 dispersal question, self-consistent) | 🔶 first answer (2c-1 + B-1): the dressed defect HELD 900 self-consistent steps (align 1.000→0.983) — but the undressed control ALSO held under the faithful kinetic, so the test doesn't yet discriminate; the decisive run is the actual M5.7 production configuration, resolution-confirmed | **G-2c-1** (roadmap M5.8.2c): M5.7 re-run on the 4D-dressed defect, full backreaction, resolution-confirmed. Fail = the model fails M5.8 |
| Fuel-shell saturation (propulsion bounded) | ❌ **FAILS for the QUADRATIC action (2026-06-05 long-horizon)**: the 900-step plateau was transient — a negative-H fuel runaway opens at τ ≈ 2 clock periods, dt-invariant, in f64≡f32, V-on/off, 24³/63³ ⇒ compactness + backreaction do NOT saturate; the field-level twin of the 2b-1 no-cap theorem and the 1D article's own βR⁴ requirement | **G-2c-2 reframed — 🔶 FIRST SANDBOX PASS (M5.8.2d, 2026-06-05 night)**: `V_u = u + βu²` on the signed spatial-curvature density (the 1D βR⁴ floor at the channel that runs) restores BOUNDED dynamics — H breathes 30–142 over τ = 48 ≈ 45 clock periods (vs 2.3 quadratic), dt-converged, clock alive, floor-bounce real; late-phase stepper stiffness + f64 confirm + Euclid control are the closeout items; the canonical-invariant question goes to Duda |
| ω self-selection (spontaneous spin-up — the time-crystal claim proper) | 🔶 partial (2c-1): the KICKED clock holds 900 steps and concentrates into the core (core-frac 0.18→0.83); spontaneous start MACHINE-ZERO in the free run (4×10⁻¹⁸). **DIAGNOSIS DELIVERED (Track C C3, 2026-06-06): PROBE BLINDNESS** — the spontaneity lives in the AMPLITUDE/breathing channel (the un-sittable minimum: `K_bb(a*) = −67.6 < 0` direct, `U″ > 0` — compulsory motion, derived variationally), and the `s(t)` clock-overlap probe is blind to it. The decisive test is now specified: UNKICKED dressed seed in the 2d quartic sandbox, probe `b_peak`/`u_min`/`H` | **G-2c-3**: from a non-spinning dressed defect, the dynamics selects a specific ω |

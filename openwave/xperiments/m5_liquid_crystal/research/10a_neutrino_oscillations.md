...
>
> **✅ N4 FIRST PASS 2026-06-21 (LOCAL, #236 HELD) , delta_CP PREDICTED MAXIMAL.** Record: [`sandbox_v10/n4_findings.md`](sandbox_v10/n4_findings.md). Adding the CP-odd CHIRAL (Lifshitz/cholesteric) coupling makes the flavour mass matrix complex Hermitian -> a genuine `delta_CP` via Jarlskog. The loop field theory **predicts `delta_CP = +-90 deg` (MAXIMAL CP violation)**, ROBUST across delta/chi/geometry (with `theta_23` = 45, `theta_12` = 35.26 held). This is the **mu-tau REFLECTION** result (Harrison-Scott: mu-tau reflection => `theta_23`=45 AND `delta_CP`=+-90) REALIZED from the loop geometry, not assumed. `theta_13` and `delta_CP` share ONE chiral origin (the loop handedness) -> the N3 `theta_13` "fit" is reframed as the chiral partner of `delta_CP`. `g_chiral`->0 recovers N3 (CP-conserving) exactly. Data leans toward `delta_CP` ~ 270 (= -90); NuFIT 212 sits between maximal and CP-conserving. **Open (the make-history crux):** is the handedness a TOPOLOGICAL invariant (loop writhe / self-linking) -> would make `theta_13` + `delta_CP` fully PREDICTED from topology.

...
| Eq.18 matrix evolution conserves energy | ✅ PASS<br>The Duda Eq.18 leapfrog is symplectic: secular drift falls 2.15% → 1.13% → 0.03% as dt → 0, confirming the production EOM conserves the Hamiltonian.<br>`m5_5_4_matrix_evolution_check.py` |
...
| Driven defect sustains an (A,ω) excess (M5.7.3) | ✅ POSITIVE<br>A continuous EM-like drive holds a bounded, frequency-selective (A,ω) excess at ~3× the free baseline at the resonant mode (N=48-confirmed) — the modulation lever for 9b thermal exists.<br>`m5_7_3_driven_oscillation.py` |
...
| Spontaneity confirmed at field level | ✅ POSITIVE<br>A damped-settled configuration restarted with P = 0 EXACT regrows kinetic energy 0 → 5.76 by τ = 4000, H conserved to 0.5%, dt/2-converged to 4 significant digits — a zero-momentum field spontaneously moves: the clock self-starts.<br>`m5_8_2g_spontaneity.py` |
...
The split that matters: **existence** (rows 1–6, all ✅ — a self-starting clock with a frequency that is a property of the state) vs **identification with the electron** (rows 7–10 — the calibration program, where the honest gaps are). "Does the ZBW clock exist?" — yes. "Is it the electron's?" — not yet, and the gap is localized.
...
| The state is quasi-periodic (not the "0.262 comb") | ⚠️ NEGATIVE-INFORMATIVE (self-caught)<br>Pre-registered scrutiny showed the earlier "strictly periodic ω₀ = 0.262 + exact comb" was an FFT-window artifact (the peak moves with window length; the comb was bin arithmetic) — the honest state is quasi-periodic: a resolved fundamental over broadband drift.<br>`sandbox_vn/m5_8_2h_omega_attractor.py` (ref mode) |
...
| Matter + light share a cone ceiling (N-4) | ✅ POSITIVE<br>The twist (KG) and both tilt (EM) channels hit the identical maximal radial signal speed c(r̂)/2 = 1.0000 at N=48 and 64 — matter and light share the maximum speed on the defect background (local SR as cone-relative kinematics); the tangential cone shapes differ per channel (one tilt polarization rank-1, non-propagating along one direction).<br>`sandbox_vn/m5_8_2k_tilt_cone.py` |
| Molten clock + thermal bridge (N-6d) | ✅ POSITIVE (classified)<br>The saturated breather is a MOLTEN CLOCK — a persistent ω₁ comb riding low-dimensional chaotic dressing (λ_max +0.4–0.7/τ, D₂ 2.7–3.0, control-anchored) whose intensity grows with excitation: the first in-data contact with the SABER per-defect-heat hypothesis (the dressing = the thermal sector).<br>`sandbox_vn/m5_8_2n_chaos_battery.py` |
| The clock regularizes toward the ground state (N-6c) | ✅ POSITIVE<br>The coldest measured state (deep-settle, T = 2.36) reads near-regular (λ_max +0.11 ≈ the estimator bias floor, D₂ 1.68 vs 2.7–3.0 hot): cold = ticking clock, hot = molten — and full coldness is FORBIDDEN by the un-sittability that drives the self-start (the clock and its heat are one object).<br>`sandbox_vn/m5_8_2o_omega_of_E.py` |
...
| Multi-defect / composites | 9d — everything above is single-defect; ensemble behavior is the thermal-medium question. |
| Strict single-line periodicity | Not claimed — the state is a molten clock (quasi-periodic + low-dim chaos), regularizing toward the cold ground state. |

---

## Reproduction

Every script above lives under `research/sandbox_v8/` (the M5.8.0–2g arc) or `research/sandbox_vn/` (the N-1…N-6e ladder). The full command table — prerequisites, run times, expected outputs — is in [`0b_M5_roadmap.md`](0b_M5_roadmap.md) § *Reproduction commands*. Each script carries its own PREREQUISITE + RESULTS docstring with the honest caveats inline.

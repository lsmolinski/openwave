...
| **3 Magnetic Field** | `N → S` (along `∇×n̂`) | honors size (`∝‖∇×n̂‖`); color: single → orange, gradient → **bluered signed N/S** (same `_curl_signed_proj` as WM7) | `director_curl_field`, via `update_em_vector_glyphs` |

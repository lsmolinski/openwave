...
def _copy_slice_xy(
    ...
    for i, j in slice_ampL:
        slice_ampL[i, j] = trackers.ampL_local_emarms_am[i, j, mid_z]
        slice_ampT[i, j] = trackers.ampT_local_emarms_am[i, j, mid_z]
...
def _copy_slice_xz(
    ...
    for i, k in slice_ampL:
        slice_ampL[i, k] = trackers.ampL_local_emarms_am[i, mid_y, k]
        slice_ampT[i, k] = trackers.ampT_local_emarms_am[i, mid_y, k]
...
def _copy_slice_yz(
    ...
    for j, k in slice_ampL:
        slice_ampL[j, k] = trackers.ampL_local_emarms_am[mid_x, j, k]
        slice_ampT[j, k] = trackers.ampT_local_emarms_am[mid_x, j, k]
...
def sample_avg_trackers(
    ...
    # ampL_local_emarms_am contains per-voxel RMS values, square them for energy
    ...
    trackers.ampL_global_emarms_am[None] = float(np.sqrt(total_ampL_squared / n_samples))
    trackers.ampT_global_emarms_am[None] = float(np.sqrt(total_ampT_squared / n_samples))
...
def update_flux_mesh_values(
    ...
    for i, j in ti.ndrange(wave_field.nx, wave_field.ny):
        ...
        ampL_value = trackers.ampL_local_emarms_am[i, j, wave_field.fm_plane_z_idx]
        ampT_value = trackers.ampT_local_emarms_am[i, j, wave_field.fm_plane_z_idx]
        ...
        if wave_menu == 1:  # greenyellow (wave_menu == 1)
            wave_field.fluxmesh_xy_colors[i, j] = colormap.get_greenyellow_color(
                psiL_value,
                -trackers.ampL_global_emarms_am[None] * 2,
                trackers.ampL_global_emarms_am[None] * 2,
            )
            ...
            wave_field.fluxmesh_xy_colors[i, j] = colormap.get_bluered_color(
                psiT_value,
                -trackers.ampT_global_emarms_am[None] * 2,
                trackers.ampT_global_emarms_am[None] * 2,
            )
            wave_field.fluxmesh_xy_vertices[i, j][2] = (
            ...
            wave_field.fluxmesh_xy_colors[i, j] = colormap.get_viridis_color(
                ampL_value, 0, trackers.ampL_global_emarms_am[None] * 2
            )
            wave_field.fluxmesh_xy_vertices[i, j][2] = (
            ...
            wave_field.fluxmesh_xy_colors[i, j] = colormap.get_ironbow_color(
                ampT_value, 0, trackers.ampT_global_emarms_am[None] * 2
            )
            wave_field.fluxmesh_xy_vertices[i, j][2] = (
                ampT_value / univ_edge_z * warp_mesh
                + wave_field.flux_mesh_planes[2] * (wave_field.nz / wave_field.max_grid_size)
            )
        elif wave_menu == 5:  # blueprint
    ...
    for i, k in ti.ndrange(wave_field.nx, wave_field.nz):
        ...
        ampL_value = trackers.ampL_local_emarms_am[i, wave_field.fm_plane_y_idx, k]
        ampT_value = trackers.ampT_local_emarms_am[i, wave_field.fm_plane_y_idx, k]
    ...
    for j, k in ti.ndrange(wave_field.ny, wave_field.nz):
        ...
        ampL_value = trackers.ampL_local_emarms_am[wave_field.fm_plane_x_idx, j, k]
        ampT_value = trackers.ampT_local_emarms_am[wave_field.fm_plane_x_idx, j, k]

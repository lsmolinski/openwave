...
def run_sweep(phase1=0.0, phase2=np.pi):
    ...
    for i, sep_lam in enumerate(seps):
        ...
        force = -np.gradient(energy, dx)
        f_wave[i] = force[idx_left] * we.FORCE_TO_N
...
def plot_sweep(results):
    """Plot force vs separation: wave force + Coulomb reference."""
    sep = results["sep_lam"]
    fw = results["f_wave"]
    ...
    ax1 = axes[0]
    ax1.plot(sep, fw, "o-", color="#44AAFF", markersize=2, linewidth=1, label="F = -∇E (wave)")
    ...
    ax2 = axes[1]
    sign_match = np.sign(fw) == np.sign(fc)
    ...
    n_ok = np.sum(sign_match)
    ntot = len(sep)
    ax2.set_title(
        f"Direction match: {n_ok}/{ntot} ({100*n_ok/ntot:.0f}%)",
        fontsize=11,
        family="Monospace",
    )
    ax2.plot(sep, np.where(sign_match, 1, -1), "o", color="#44AAFF", markersize=3)
    ...
    ax3 = axes[2]
    ax3.semilogy(sep, np.abs(fw), "o-", color="#44AAFF", markersize=2, linewidth=1, label="|F wave|")
    ...
    ax3.set_title(
        "Log magnitude — should follow Coulomb 1/r² slope", fontsize=11, family="Monospace"
    )

...
- **Connection to granule dynamics and time**: Density in the medium is directly related to granule velocity — the speed at which granules cycle through their elliptical motion (90° phase-shifted from displacement). A density deficit means slower granule cycling → lower local frequency → longer λ → slower rate of change (time dilation). This links Smoliński's buoyancy mechanism to gravitational time dilation: time dilation near massive bodies emerges from the same density/pressure deficit that produces the gravitational force itself (the chain: **movement → granule velocity → density/pressure → frequency → time**).

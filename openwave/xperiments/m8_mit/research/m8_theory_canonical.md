...
| Λ from the edge mode | `Λ_ref = 3/R²`, the eigenvalue `2/R²` carried by the Gauss-equation factor 3/2 (3 = Ricci trace derived, 1/2 imported from GR): a reference value, whose identification with the physical `Λ` is open, gated on an undetermined stress tensor | Λ ground-mode paper [10.5281/zenodo.18307314](https://doi.org/10.5281/zenodo.18307314); the reference-value reading: [cosmological-constant.md](https://github.com/dmobius3/mode-identity-theory/blob/main/files/cosmos/files/cosmological-constant.md) § IV (author repo) | 🚧 the 2/R² input is now verified (M8.1 ✅); the Gauss-equation 3/2 step and the R-problem (§ 6) remain untested |
...
| The top-multipole extrema are unique | on the unit sphere of `V₃`, `r̂₆` attains its maximum `463/924` exactly on the rotation-and-phase orbit of the hexagon `(v₃ + v₋₃)/√2`, and its minimum `1/924` exactly on the coherent states; the equality set of the nematic bound `λ_max ≤ 15/√6` is exactly three points, each reached by two of the three blocks, with `M₃` tangent to the bound | platform task M8.13, author-proposed ([#583](https://github.com/openwave-labs/openwave/pull/583)) | ✅ AUDITED ARGUMENT (M8.13, 2026-09-22): the auditor's own route, then the author's graded step by step with its supplied parts named, no GAP or DEFECT; an adversarial audit refuted nothing. ⚠️ An audited argument, never a verified theorem, and no stability claim ([`tasks/m8_13_task_details.md`](tasks/m8_13_task_details.md)) |
...
| --- | --- |
| Cycle 2 (α) | α is input AND output: it calibrates R, then the scaling law "predicts" α. The 0.4% match is a consistency check; the non-circular content is downstream, the conditional reference value `Λ_ref = 3/R²` about 23% below `Λ_obs`, a loose comparison rather than a forward prediction, whose 5σ test stays unscored until R carries an error budget of its own |
| Cycle 7 (the R-problem) | the two independent routes to R disagree: the coupling route, the best-conditioned, gives ≈ 6.1 Gpc, and the coarser mass route ≈ 20 Gpc on the canonical electron-muon pair (3.2×, ~10.5× in `Λ_ref`) or ≈ 10.5 Gpc on the best assigned pair, muon-top (1.7×, ~2.9×). The displacement is of the size the mass inversion's conditioning can produce, but its origin is unresolved. Every R-dependent number inherits this until one route is shown correct (OQ2) |
...
| OQ2 | The R-problem: the coupling route (≈ 6.1 Gpc, best-conditioned) and the coarser mass route (≈ 10.5 to 20 Gpc, by pair) do not meet. Is the displacement the mass inversion's conditioning or a structural inconsistency, and can a route give `R` an error budget of its own, which the Λ coefficient test needs before it can be scored? | author-side; M8.1 and M8.3 have landed (✅), and no platform task has bounded it yet | 🚧 open |

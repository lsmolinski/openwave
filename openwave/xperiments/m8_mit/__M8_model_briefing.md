...
| EM | α read from the first Fibonacci well (13/60) at grid depth. Calibration-webbed: α is the calibration input that fixes the curvature radius R, so the 0.4% match is a consistency check, not a free prediction (the author's own Cycle 2) |
| Quantum | not applicable: no field quantization. MIT is spectral geometry on a fixed manifold, not a QFT |
| Gravity | Einstein's equations unchanged. The Möbius-surface first-positive eigenvalue `2/R²` is the spectral seed (✅ M8.1); under the stated Gauss lift it becomes `6/R²`, and with the de Sitter `½` imported from GR the reference value is `ΛR² = 3`. Two gaps remain: the value of R is the open R-problem (two routes disagree by 3.2× on the canonical pair and 1.7× on the best assigned pair, author-flagged), and the physical identification of `Λ` is gated on an undetermined stress tensor. Test, stated in advance of Euclid DR1: `Λ_obs R_ind² = 3`, where >5σ falsifies that identification and not the seed. It cannot be scored until a route to R brings an error budget of its own and fixes its choices on grounds independent of this comparison, before the scoring run; until then the α route returns the conditional reference value `3/R²` about 23% below `Λ_obs` |
...
| Next falsifier | Euclid DR1 (full release, mid 2027): a₀(z) ∝ H(z), Λ epoch-independence, and the sign-fixed negative (1+z)¹ term in H²(z). The stellar mass function at z ≳ 10 (Row IV) can score earlier: since its scoring rule was set on 2026-09-23 it scores on the first qualifying Euclid measurement, from DR1-Foundation data (November 2026, tentative) or later, and the JWST measurements examined lie inside its registered ceiling |
...
| Attribute | M8 |
| --- | --- |
| Free parameters | one calibration anchor per sector (H₀, Λ-or-α, m_e), plus the four named selection choices above. The ledger's shared-freedom audit finds several are not forced: the well set {13, 21, 34, 55} is never extremal under its own eight-functional variational test (selection null; the ledger withdrew its rank position, which independent implementations move); the torsion-to-slot map is a pre-registered null (`mass-null-v1.1`, corrected table, p_A = 0.690; supersedes `mass-null-v1.0`'s p = 0.174 on the pre-correction table); α is input-and-output (Cycle 2, consistency check); and the two independent routes to R disagree by 3.2× on the canonical pair and 1.7× on the best assigned pair (Cycle 7, the framework's most significant internal tension) |
...
| Falsifiable near-term tests | Euclid DR1 is the live gate, with pre-registered thresholds on a five-row contender card: four rows read the full release (mid 2027), and Row IV scores on the first qualifying measurement, from DR1-Foundation data or later; the JWST measurements examined lie inside Row IV's registered ceiling |
...
| Charge / color quantization | 🚧 structural in MIT (2I stabilizers); not run in the engine, and no task is queued |
...
| Native field dynamics | 🚧 absent, the defining open problem and the M8 program's core: MIT has no Lagrangian, so masses are assigned by structure rather than emerging from evolution. The platform supplies the Lagrangian-family candidates and the simulation engineering ([`research/m8_platform_pointers.md`](research/m8_platform_pointers.md)). No route is live in the column ([#512](https://github.com/openwave-labs/openwave/discussions/512)), and the author's own construction of an action is parked, with one entry test for a candidate from any source ([stress-tensor bridge § IX](https://github.com/dmobius3/mode-identity-theory/blob/main/files/framework/files/working/files/stress-tensor-bridge.md#ix-parked)) |
...
| M8.12 | ✅ EVERY FROZEN CLAIM REPRODUCES, `G2`'s uniqueness clause unresolved (2026-09-21), audited by M8.13 below: the reduced Morse census of the level-6 quartic `r̂₆`, ten critical orbits with their exact values, transverse dimensions, signatures and characteristic polynomials, the `v₁` kernel and both ends, blind, two agents and an adversarial audit that found no mathematical defect. Sector-independent by D7. **No stability claim**: this is the Morse index of the leading reduced quartic transverse to the phase-and-rotation orbit, not the stability of a branch germ. No MODELS.md cell moves and M8.7's gate is unchanged ([`research/findings/m8_12_method_note.md`](research/findings/m8_12_method_note.md)) |
| M8.13 | ✅ AUDITED ARGUMENT (2026-09-22): the hexagon orbit is the only maximizer of `r̂₆` on the unit sphere of `V₃`, and the coherent states the only minimizers. An offline auditor's own argument, committed before it saw the author's, then a step-by-step grading of the author's note with every supplied part named; four independent derivations of the equality set and an adversarial audit that refuted nothing. **An audited argument, not a verified theorem, and no stability claim.** No MODELS.md cell moves and M8.7's gate is unchanged ([`research/tasks/m8_13_task_details.md`](research/tasks/m8_13_task_details.md)) |
...
| --- | --- |
| **Does any candidate field theory here derive MIT's source fingerprint?** | Every other column in this arena evolves a Lagrangian; MIT has none, and that is its central gap. What is new is that the target is pinned rather than gestured at. Supply an ACTION with its metric coupling, and therefore a defined `T_μν`: an equation of motion alone does not fix a stress tensor, and an EOM plus a separately chosen `T_μν` tests the choice, not the theory. Restrict the theory to the static `S³` or the `S³/2I` quotient, whichever your construction needs; that is a choice of arena, not the fork. Separately, declare which metric your `T_μν` sources: physical-static or effective-metric, branches (a) and (b) of § III. Branch (c) there, shifted coefficient, is an outcome about the coefficient rather than a metric to source, so it is not a third placement to declare here. That sourcing choice is open, and it is not ours to assume on your behalf. **For the effective-metric placement with `a_eff = a₀S` held**, the target is pinned: ONE tied object, `Ψ²/S³`, whose `S⁻³` and `S⁻¹` sectors carry coefficient ratio exactly `−1` rather than an independently normalized `w = −2/3` component; PLUS a separate genuine `w = −1` constant; with NO `S⁻²` sector on flat effective slices. Fingerprint, notation and conditionality: [stress-tensor-bridge.md](https://github.com/dmobius3/mode-identity-theory/blob/main/files/framework/files/working/files/stress-tensor-bridge.md) § VI, "First results: the pinned target". The author's own construction of this object is parked, and [§ IX](https://github.com/dmobius3/mode-identity-theory/blob/main/files/framework/files/working/files/stress-tensor-bridge.md#ix-parked) states the one entry test a candidate meets from any source: C1 to C11 and the gates § VI registers, the McKay slot target frozen in M8.2 § 2 if the same action is offered as this column's field dynamics, and the further conditions § IX names for a candidate that also claims the clock or the coefficient. A NO on a proposed theory is a result and will be recorded as one: it closes that route rather than being tuned into a YES, and it is not a proof that no such theory exists |
| **Can `R` be fixed independently of `Λ`, the CMB, and de Sitter, with an error budget of its own?** | `Λ_ref = 3/R²` can be checked against the measured `Λ` only if `R` arrives from elsewhere; reading `R` off `Λ` makes the whole chain a tautology. Two author-side routes are live: measured `α` → `Ω_Λ` → `R`, the best-conditioned, with `Λ_ref` about 23% below `Λ_obs`, and the fermion mass spectrum, coarser, at order of magnitude. An independent central value is not enough: neither route states an uncertainty on `R` apart from the comparison it would feed, so the Λ page's test, `Λ_obs R_ind² = 3` at 5σ, cannot yet be scored ([the coefficient test](https://github.com/dmobius3/mode-identity-theory/blob/main/files/framework/files/working/files/r-problem.md#the-coefficient-test)). Wanted, either way round: a third route that touches none of the three excluded inputs, states its own uncertainty, and fixes every choice its reading depends on, on grounds independent of this comparison and before the scoring run; or a hostile audit that kills one of the two live routes by exhibiting a hidden dependence on them. [r-problem.md](https://github.com/dmobius3/mode-identity-theory/blob/main/files/framework/files/working/files/r-problem.md) |

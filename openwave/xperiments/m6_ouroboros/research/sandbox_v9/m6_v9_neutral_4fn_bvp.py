"""M6 sandbox v9 — Neutral 4-function BVP, Interpretation A (2026-05-22).

Forked from sandbox_v6/m6_v6_4fn_calibrated_bvp.py per Paul's 2026-05-21 PM
reply (via DeepSeek):
...
Interpretation A: 4-function (V, A, Q, J) BVP with Q_CS=0 (neutral) + same
asymmetric helicity init as the charged electron.

CHANGES FROM v6:
  - I_TARGET = 0.0  (Q_CS = 0 for neutral, was 1.0 for charged)
  - Use --no-warm-start by default (exp(-r) asymmetric seed: V,Q>0; A,J<0)
  - Try ω_init = 0 (no time periodicity, true static) AND ω_init = 1 (rest mass = m_e)

KEY QUESTION: does the BVP find a non-trivial neutral chaoiton, or does it
collapse to trivial (all-fields-zero) under the Q_CS=0 constraint?

ORIGINAL v6 HEADER FOLLOWS (kept for reference):
================================================
...
# v9 NEUTRAL: I_TARGET = 0.0 (Q_CS = 0 for neutral chaoiton).
I_TARGET = 0.0
